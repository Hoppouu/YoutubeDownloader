...
def _download_file(
    ...
    if not destination.is_file() or destination.stat().st_size == 0:
        raise DependencyError("다운로드한 파일이 비어 있습니다.")
    ...
    def update(self, expected_version=None):
        ...
        try:
            ...
            if not _has_windows_executable_header(staged_path):
                raise DependencyError("다운로드한 yt-dlp 파일이 올바른 Windows 실행 파일이 아닙니다.")
            ...
            if result.returncode != 0 or not downloaded_version:
                raise DependencyError("다운로드한 yt-dlp 실행 파일 검증에 실패했습니다.")
        ...
        try:
            with _managed_temp_directory("youtube-downloader-ffmpeg-") as temp_dir:
                ...
                try:
                    ...
                    raise DependencyError(f"FFmpeg 압축 파일을 처리하지 못했습니다: {exc}") from exc

            if not self._validate_executable(staged_ffmpeg):
                raise DependencyError("다운로드한 ffmpeg.exe 검증에 실패했습니다.")
            if not self._validate_executable(staged_ffprobe):
                raise DependencyError("다운로드한 ffprobe.exe 검증에 실패했습니다.")
        ...
        try:
            with _managed_temp_directory("youtube-downloader-deno-") as temp_dir:
                ...
                try:
                    ...
                    raise DependencyError(f"Deno 압축 파일을 처리하지 못했습니다: {exc}") from exc

            if not self._is_supported(staged_path):
                raise DependencyError("다운로드한 deno.exe 검증에 실패했습니다.")
            os.replace(staged_path, self.executable_path)
